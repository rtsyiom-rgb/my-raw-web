    ...
    res.send(`
        <body style="background:#0f111a; color:white; font-family:sans-serif; display:flex; align-items:center; justify-content:center; height:100vh; margin:0;">
            <div style="background:#1a1c2e; padding:30px; border-radius:15px; border:1px solid #2d304d; width:400px; box-shadow: 0 10px 30px rgba(0,0,0,0.5);">
                <h2 style="color:#5765f2; text-align:center; margin-bottom:20px;">SCRIPT HUB</h2>
                <form action="/save" method="POST">
                    <input type="text" name="title" placeholder="ชื่อไฟล์ (เช่น BloxFruits.lua)" style="width:100%; padding:10px; margin-bottom:10px; background:#0f111a; border:1px solid #333; color:white; border-radius:5px;" required>
                    ...
                    <button type="submit" style="width:100%; padding:15px; background:#5765f2; border:none; color:white; font-weight:bold; margin-top:20px; cursor:pointer; border-radius:5px; transition:0.2s;">GENERATE</button>
                </form>
            </div>
        </body>
    `);
...
// --- หน้าแสดงผล (เน้นแค่ loadstring ตามรูปที่ 5) ---
app.get('/:id.lua', (req, res) => {
    ...
    // ตรวจสอบว่าเป็นเบราว์เซอร์หรือไม่
        ...
        return res.send(`
            ...
            <head>
                ...
                <style>
                    ...
                    .code-box { font-family: 'Consolas', monospace; font-size: 16px; color: #dcdccc; white-space: pre-wrap; word-break: break-all; text-align: left; }
